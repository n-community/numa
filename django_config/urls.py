...
from controllers.notfound   import NotFoundPage
...
a = list(MapPage.get_methods)
b = list(MapPage.post_methods)
map_actions = "|".join(a + b)
...
urlpatterns = [
    ...
    re_path(r"^sundayn(?:/([0-9]+))?(?:/(feedback))?$", SundayNPage.as_view()),
    ...
    re_path(r"^user(?:/([^/]+)(?:/(edit))?)?$", UserInfoPage.as_view()),
