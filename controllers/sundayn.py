...
class SundayNPage(lib.BaseHandler):
  def get(self, num=None, action=None):
    ...
    if action == "feedback":
      self.ShowFeedback(template_values, sn)
    else:
      self.ShowSundayN(template_values, sn)

  def ShowSundayN(self, template_values, sn):
    template_values["archive_nums"] = range(1, template_values["last_sundayn"].number + 1)
    self.RenderTemplate("sundayn.html", template_values)
  ...
  def ShowFeedback(self, template_values, sn):
    ...
    self.RenderTemplate("sundayncomments.html", template_values)
